...
class HttpService {
        private getAuthHeadrer(): Record<string, string> {
        ...
        private getHeadrer(auth: boolean = true): Record<string, string> {
                if (auth) {
                        return this.getAuthHeadrer();
                }
                return {
                        'Content-Type': 'application/json',
                }
...
// Export the singletion instance
...
// bind create a new function where this is permanently ste to the instance of HttpService


export const getWithAuth= httpService.getWithAuth.bind(httpService);
export const psotWithAuth= httpService.postWithAuth.bind(httpService);
